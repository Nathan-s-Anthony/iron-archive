      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 lg:grid-rows-1">
        {items.map((item, id) => {
          return (
            <div
              key={id}
              className="lg:w-full w-full lg:h-130 group h-100 overflow-hidden relative cursor-pointer"
            >
              <div className="left-5 top-5">
                <Pill title={item.type} />
              </div>
              <div className="flex left-5  absolute bottom-5 w-full z-40">
                <div className="border-b border-primary/30 w-full">
                  <span className="text-secondary uppercase">
                    {item.faction} FORCES
                  </span>
                  <h6 className="text-primary font-sans text-3xl">
                    {item.name}
                  </h6>
                  <p>{item.designation}</p>
                </div>
              </div>
              <div className="collection-overlay block w-full h-full z-30 absolute"></div>
              <Image
                alt="jet"
                className="object-cover object-center  group-hover:scale-115 cursor-pointer transition-all duration-300"
                fill
                title={item.imageAlt}
                src={item.image}
              />
